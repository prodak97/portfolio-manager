...
const TechnologyIcon = ({ name }: { name: string }) => {
  const iconProps = { className: "inline-block w-6 h-6 mr-2", title: name };
    switch (name.toLowerCase()) {
      ...
      case 'next.js': return <RiNextjsFill {...iconProps} />;
      case 'Microservices': return <svg href="/img/icons/microservices.svg"{...iconProps} />;
      case 'CI/CD': return <svg href="/img/icons/cicd.svg"{...iconProps} />;
      default: return null;
    }
  };
...
const RuslanPortfolio = () => {
  ...
  const downloadCV = async () => {
    try {
      ...
      cvNode.style.fontFamily = "Inter, system-ui, -apple-system, Segoe UI, Roboto, 'Helvetica Neue', Arial, 'Noto Sans'";
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <nav className="fixed top-0 left-0 w-full bg-white shadow z-50 flex items-center justify-between px-8 py-4">
        ...
        <div className="flex gap-6 items-center">
          ...
          <button className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 font-medium" onClick={downloadCV}>Download CV</button>
        </div>
      </nav>
